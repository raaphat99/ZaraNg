...
export class FilterService extends ApiService {
  ...
  fetchProductsByMaterial(categoryId: number, material: string): Observable<any> {
  ...
  fetchProductsByMultipleCategories(categoryIds: number[]): Observable<any[]> {
    const requests = categoryIds.map(id => this.httpClient.get<any[]>(`${this.apiUrl}/category/${id}`));
    return requests.length > 1 ? this.httpClient.get<any[]>(`${this.apiUrl}/category/${categoryIds.join(',')}`) : requests[0];
  }
}
